  ...
  <div class="box">
    <div v-if="!showChat" class="sub-boxa">
      ...
      <mt-loadmore
        ...
        :bottom-method="loadBottom_mail"
        :bottom-all-loaded="allLoaded_mail"
        bottomPullText
        ref="loadmore_mail"
      >
        <div class="userlist">
      ...
      <div v-else class="no-user">暂无用户加入聊天</div>
    </div>
...
// 添加浏览器通知
let isPermissionNotification = false;
document.addEventListener("DOMContentLoaded", function() {
  if (!Notification) {
    MessageBox.alert("您的浏览器不支持提示，试试谷歌浏览器。", "提示");
    return;
  }
...
export default {
  ...
  mounted() {
    ...
    this.sockets.subscribe("connect", () => {
      this.$socket.emit(
        "login.event",
        { docId: this.docId, password: this.password },
        res => {
          ...
          this.doctorInfo = res;
          this.doctorAvatar = res.poster
            ? res.poster
            : require("../assets/images/chat/my_avatar.png");
        }
      );
    });
    ...
    this.sockets.subscribe("leave.event", data => {
      ...
      if (
        ...
        data[0].userId === this.currentUser.userId
      ) {
        Toast("对方已离线");
      }
      ...
      if (isMe) {
        ...
        if (data.fromUser == this.currentUser.userId) {
          ...
          if(isPermissionNotification) {
            let notification = new Notification('来自'+data.fromUser+'的新消息', {
              body: data.message,
